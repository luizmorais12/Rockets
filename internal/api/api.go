...
func (h apiHandler) handleReactToMessage(w http.ResponseWriter, r *http.Request) {
	...
	data, _ := json.Marshal(response{Count: reactionCount})
	w.Header().Set("Content-Type", "application/json")
	_, _ = w.Write(data)
}
...
func (h apiHandler) readRoom(w http.ResponseWriter, r *http.Request) (pgstore.Room, uuid.UUID, uuid.UUID, bool) {
	rawRoomID := chi.URLParam(r, "room_id")
	roomID, err := uuid.Parse(rawRoomID)
	if err != nil {
		http.Error(w, "invalid room id", http.StatusBadRequest)
		return pgstore.Room{}, uuid.Nil, uuid.Nil, false
	}
	...
	if err != nil {
		http.Error(w, "invalid message id", http.StatusBadRequest)
		return room, uuid.Nil, uuid.Nil, false
	}

	return room, roomID, messageID, true
}
